...
void InitializePFLOptionBits( unsigned int option_bit_offset ) // touch the PFL Option Register
{
  ...
  flash_fd = alt_flash_open_dev( EXT_FLASH_NAME );
    ...
    if( option_bits_buffer != NULL )
    {
      ...
      page_end = page_start + BYTES_FOR_HW_IMAGE;

      option_bits_buffer[FACTORY_HW_IMAGE_PAGE] = ( page_end << 4 ) | ( page_start >> 12 );

      // User image
      page_start =  USER1_HW_IMAGE_OFFSET;

      page_end = page_start + BYTES_FOR_HW_IMAGE;

      option_bits_buffer[USER1_HW_IMAGE_PAGE] = ( page_end << 4 ) | ( page_start >> 12 );

      // preserve User image2 so it will also configure
 //
 //     page_start =  USER2_HW_IMAGE_OFFSET;
 //
 //     page_end = page_start + BYTES_FOR_HW_IMAGE;
 //
 //     option_bits_buffer[USER2_HW_IMAGE_OFFSET] = ( page_end << 4 ) | ( page_start >> 12 );


#ifdef DEBUG_MODE
    printf("DEBUG: touched USER2_HW_IMAGE PFL Option Bits\n");
#endif

      // The byte following the 128-byte option bit section must be 0x3
      // These values indicate the "version" of file image being used
//      option_bits_buffer[32] = 0xFFFFFF02;  // version 2 : old 8.x Pofs
    option_bits_buffer[32] = 0xFFFFFF03;  // version 3 : NEW 9.x  Pofs
