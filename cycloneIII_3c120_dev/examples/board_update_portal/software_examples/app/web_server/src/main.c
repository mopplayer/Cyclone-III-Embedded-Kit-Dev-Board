...
printf( "DEBUG: LEDTask \n" );
  ...
  {
    // Flip the led
    led_value ^= 0x8; // was 0x1 but LED0 is less preferable
...
void LCDTask( void* pdata )
{
  ...
  while( 1 )
  {
    ...
    lcd_dev = fopen( "/dev/lcd", "w" ); // LCD name in my system.h file
#ifdef DEBUG_MODE
...
#ifdef UPDATE_PFL
  // Before we do anything, make sure the PFL option bits are initialized correctly. but... why?
