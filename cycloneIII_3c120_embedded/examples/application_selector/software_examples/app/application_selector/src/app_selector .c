...
#include "srec.h"
//#include "flash_spansion_s29gln.h"
#include "sd_card.h"
 ...
 *
 *           The cache works by storing the "AS_MAX_FLASH_PAGES" most
 *           recently loaded hardware images in fixed pages in flash starting
 *           at "AS_APP_HW_IMAGE_OFFSET_START".  The ZSFA catalog holds the
 *           information about the timestamp value of each hardware image
 *           stored in one of those pages.  When it's time to load a new
                         ...
                         alt_touchscreen* touchscreen )
{
  ...
  sw_flash_offset = AS_APP_SW_IMAGE_OFFSET;
  hw_flash_offset = AS_APP_HW_IMAGE_OFFSET_START;
    ...
    if( !dir_listing->error )
    {
      ...
      if( err == OS_NO_ERR )
      {
        if( AsGetFullFileName( hw_filename, "_hw.flash", dir_listing ) == AS_SUCCESS )
        {
          ...
          {
            hw_flash_page = AsFindFlashLocationForHWImage();
            total_bytes += AsGetFileSize( hw_filename );
          }
        }
        else if( AsGetFullFileName( hw_filename, "_hw.rbf", dir_listing ) == AS_SUCCESS )
        {
        	need_to_load_hw_image = 1;
        	filetype = AS_RBF;
          hw_flash_page = AsFindFlashLocationForHWImage();
        ...
        if( need_to_load_hw_image )
        {
          hw_flash_offset = AsConvertFlashPageToOffset( hw_flash_page );
          ret_code |= AsCopyFileFromSDCardToFlash( hw_filename,
                                                   hw_flash_offset,
                                                   display,
                                                   touchscreen,
                                                   &prog_window,
          ...
          if( timestamp > 0 )
          {
            AsRecordHWImageInZSFACatalog( hw_flash_page, timestamp );
          }
        }

        //Copy the SW image from SD Card to flash.
        ret_code |= AsCopyFileFromSDCardToFlash( sw_filename,
                                                 sw_flash_offset,
                                                 display,
                                                 touchscreen,
                                                 &prog_window,
...
{
  return( AS_SELECTOR_HW_IMAGE_OFFSET + ( hw_flash_page * 0x380000 ));
}
  ...
  if( flash_fd )
  {
    // Traverse the catalog until we find an unused spot.
      ...
      if( *flash_ptr == 0xFFFFFFFF )
      {
        ...
        ret_code = flash_spansion_s29gln_write ( EXT_FLASH_BASE, (int)(flash_ptr) - EXT_FLASH_BASE, (void*)(&timestamp), 2 );
      	ret_code |= flash_spansion_s29gln_write ( EXT_FLASH_BASE, (int)(flash_ptr + 1) - EXT_FLASH_BASE, (void*)(&hw_offset), 2 );
#elif USE_HAL_FLASH_DRIVER
        alt_write_flash_block( flash_fd, (int)(flash_ptr) - EXT_FLASH_BASE, (int)(flash_ptr) - EXT_FLASH_BASE, (void*)(&timestamp), 4 );
        alt_write_flash_block( flash_fd, (int)(flash_ptr + 1) - EXT_FLASH_BASE, (int)(flash_ptr + 1) - EXT_FLASH_BASE, (void*)(&hw_offset), 4 );
#endif
      	break;
      }
      flash_ptr += 2;
...
int AsRefreshCatalog( void )
{
  ...
  if( flash_fd )
  {
//    flash_spansion_s29gln_erase ( EXT_FLASH_BASE, AS_FLASH_IMAGE_CATALOG_OFFSET, AS_FLASH_SECTOR_SIZE );
    alt_erase_flash_block( flash_fd, AS_FLASH_IMAGE_CATALOG_OFFSET, AS_FLASH_SECTOR_SIZE );
  }

  return( AS_SUCCESS );
}

int AsFindImagesInFlash( unsigned int pages_in_use[] )
{
  int i;
  ...
  for( i = 0; i < AS_FLASH_IMAGE_CATALOG_SIZE; i += 8 )
  {
    ...
    else if( *flash_ptr != 0x00000000 )
    {
      ...
      pages_in_use[num_images_in_flash] = *(flash_ptr + 1);
      num_images_in_flash++;
    }
    flash_ptr += 2;
  }
 ...
 *           page in flash was written least recently.  It then erases that
 *           catalog entry so a new one can be written once the new hw image
 *           is copied to that page.
 *
...
int AsFindFlashLocationForHWImage( void )
{
	...
	unsigned int pages_in_use[AS_MAX_FLASH_PAGES];
  unsigned int* flash_ptr;
  unsigned int page_is_free;
  alt_flash_fd* flash_fd;
  ...
  if( flash_fd )
  {
    num_images_in_flash = AsFindImagesInFlash( pages_in_use );

    // If there are fewer images in flash than AS_MAX_FLASH_PAGES, then it means
    // we have at least one unused page.  Let's find it.
    if( num_images_in_flash < AS_MAX_FLASH_PAGES )
    {
    	// Index through all the pages where an image can reside, and cross-check them
    	// against the array of used pages we collected above.  When we come across
    	// a page which is not in the array, it means we've found an empty page.
    	for( hw_page = AS_FIRST_APP_HW_IMAGE_PAGE; hw_page < ( AS_FIRST_APP_HW_IMAGE_PAGE + AS_MAX_FLASH_PAGES ); hw_page ++ )
    	{
    		page_is_free = 1;
    		for( i = 0; i < num_images_in_flash; i++ )
    		{
    			if( hw_page == pages_in_use[i] )
    			{
    				page_is_free = 0;
    				break;
    			}
    	...
    	if( !page_is_free )
    	{
    		// This should technically never happen, because to get here,
        // there had to be fewer images in flash than there were pages.
        // but we should flag it just in case.
    		ret_code = AS_FAILURE;
    	}
    }
    else
  	// If there are AS_MAX_FLASH_PAGES in flash, we need to decide which one to
  	...
  	// history.  So instead, we'll just overwrite the image copied to flash the
  	// longest ago, which will be the first one we find in the image catalog.
    {
  	  ...
  	  for( i = 0; i < AS_FLASH_IMAGE_CATALOG_SIZE; i += 8 )
      {
        ...
        else if( *flash_ptr != 0x00000000 )
        {
        	...
        	// Mark catalog entry as spent because we're going to create a new one for this page.
#if USE_SPANSION_FLASH_DRIVER
         	ret_code |= flash_spansion_s29gln_write ( EXT_FLASH_BASE, (int)(flash_ptr) - EXT_FLASH_BASE, (void*)(&catalog_spent_mask), 4 );
#elif USE_HAL_FLASH_DRIVER
          alt_write_flash_block( flash_fd, (int)(flash_ptr) - EXT_FLASH_BASE, (int)(flash_ptr) - EXT_FLASH_BASE, (void*)(&catalog_spent_mask), 8 );      	break;
#endif
        }
...
//  sdcard_fd = SDOpen( filename, F_READ );
  sdcard_fd = SDOpen( filename, O_RDONLY );
...
/*****************************************************************************
 *  Function: AsCopyFileFromSDCardToFlash
 *
...
//  sdcard_fd = SDOpen( filename, F_READ );
  ...
  else
  {
  	...
  	// First pre-erase the flash sectors we are about to write.
//    ret_code |= flash_spansion_s29gln_erase ( (int)flash_fd->base_addr, flash_offset, AS_BYTES_FOR_HW_IMAGE );

	  AsGetFileExtension( filename, file_ext );
	    ...
	    do
      {
	      ...
	      while( srec_decode( &srec, srec_line_ptr ) == SREC_OK )
	      {
	        if( srec.type != 0 )
	        {
	          ...
	          {
#if USE_SPANSION_FLASH_DRIVER
	            ret_code |= flash_spansion_s29gln_erase ( (int)flash_fd->base_addr, flash_offset, AS_FLASH_SECTOR_SIZE );
	            ret_code |= flash_spansion_s29gln_write ( (int)flash_fd->base_addr, flash_offset, (void*)data_buffer, AS_FLASH_SECTOR_SIZE / 2 );
#elif USE_HAL_FLASH_DRIVER
	            alt_erase_flash_block( flash_fd, flash_offset, AS_FLASH_SECTOR_SIZE );
	            alt_write_flash_block( flash_fd, flash_offset, flash_offset, (void*)data_buffer, AS_FLASH_SECTOR_SIZE );
#endif

//	            int debug_match = memcmp( data_buffer, (void*)((int)flash_fd->base_addr + flash_offset), AS_FLASH_SECTOR_SIZE );
//	            if( debug_match )
//	            {
//	              printf(" ERROR: flash does not match buffer\n" );
//	              while(1);
//	            }
	  ...
	  else if( !strcmp( file_ext, "rbf" ))
	  {
	    do
	    {
	      // Fill (or refill) the data buffer from SD Card
	      bytes_in_data_buffer = SDRead( data_buffer, AS_FLASH_SECTOR_SIZE, sdcard_fd );

#if USE_SPANSION_FLASH_DRIVER
        ret_code |= flash_spansion_s29gln_erase ( (int)flash_fd->base_addr, flash_offset, AS_FLASH_SECTOR_SIZE );
        ret_code |= flash_spansion_s29gln_write ( (int)flash_fd->base_addr, flash_offset, (void*)data_buffer, bytes_in_data_buffer / 2 );
#elif USE_HAL_FLASH_DRIVER
        alt_erase_flash_block( flash_fd, flash_offset, AS_FLASH_SECTOR_SIZE );
	      alt_write_flash_block( flash_fd, flash_offset, flash_offset, (void*)data_buffer, bytes_in_data_buffer );
#endif

//        int debug_match = memcmp( data_buffer, (void*)((int)flash_fd->base_addr + flash_offset), bytes_in_data_buffer );
//        if( debug_match )
//        {
//          printf(" ERROR: flash does not match buffer\n" );
//          while(1);
//        }
        ...
        *total_bytes_complete += bytes_in_data_buffer;
...
//  sdcard_fd = SDOpen( fullpath, F_READ );
  sdcard_fd = SDOpen( fullpath, O_RDONLY );
  if( sdcard_fd >= 0 )
  {
...
//  sd_file = SDOpen( filename, F_READ );
  sd_file = SDOpen( filename, O_RDONLY );
...
int AsGetFileExtension( char filename[], char file_ext[] )
{
  ...
  {
    pointer_to_file_extension = strrchr( filename , '.' );
    if( pointer_to_file_extension )
    {
      pointer_to_file_extension++;
