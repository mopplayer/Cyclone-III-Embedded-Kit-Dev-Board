                             ...
                             unsigned char *sectorbuf)
{
    ...
    rc = sls_sd_write_sectors(IF_MV2(fce->fat_vol->drive,)
                           secnum, 1,
...
static int update_fsinfo( void )
{
    ...
    /* update fsinfo */
    rc = sls_sd_read_sectors( fat_bpb->startsector + fat_bpb->bpb_fsinfo, 1,(short*)fsinfo);
    if (rc < 0)
    {
    ...
    *intptr = (fat_bpb->fsinfo.nextfree);
    ...
    rc = sls_sd_write_sectors( fat_bpb->startsector + fat_bpb->bpb_fsinfo,1,(short*)fsinfo);
                              ...
                              long fatsector, bool dirty)
{
    ...
    if(!fce->inuse)
    {
        rc = sls_sd_read_sectors(IF_MV2(fat_bpb->drive,)
...
int fat_find_bootsector(  unsigned char *buf )
{
  ...
  for (i=0; i<256; i++)
  {
    sls_sd_read_sectors( i, 1, (unsigned short*)buf );
...
int fat_mount(IF_MV2(int volume,) IF_MV2(int drive,) long startsector)
{
    ...
    /* Read the sector */
    rc = sls_sd_read_sectors(IF_MV2(drive,) startsector,1,(unsigned short*)buf);
    if(rc)
    {
    ...
    */
...
#endif /* #ifdef HAVE_FAT16SUPPORT */
    {
        ...
        rc = sls_sd_read_sectors(IF_MV2(drive,)
                    ...
                    unsigned long start, long count, char* buf, bool write )
{
    ...
    if (write) {
        ...
        rc = sls_sd_write_sectors(IF_MV2(fat_bpb->drive,)
